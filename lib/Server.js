...
function getAllReactNativeExternals() {
  ...
  var reactNativeModulePaths = [
    ...
    path.join(reactNativePath, 'node_modules/react-tools/src'),
  ];
  var reactNativeExternals = {};
  var externals;
  reactNativeModulePaths.forEach(function(modulePath) {
    externals = getReactNativeExternals(modulePath);
    for (var ext in externals) {
      if (externals.hasOwnProperty(ext)) {
        reactNativeExternals[ext] = externals[ext];
      }
    }
  });
...
function Server(options) {
  ...
  this.entryDir = path.resolve(__dirname, '../../../'); // FIXME: now writing in project root
...
Server.prototype = {
  ...
  _writeEntryFile: function(entryModules) {
    ...
    if (entryModules && entryModules.length > 0) {
      source += entryModules.map(function(mod) {
        return 'require("' + mod + '");'
      }).join('\n');
    }
  ...
  _startWebpackDevServer: function() {
    return new Promise(function(resolve, reject) {
      ...
      var reactNativeExternals = getAllReactNativeExternals();
      ...
      compiler.plugin('done', function(stats) {
        ...
        var allModules = stats.compilation.modules;
        var entryModules = getEntryModules(allModules, reactNativeExternals);
        this._writeEntryFile(entryModules);
